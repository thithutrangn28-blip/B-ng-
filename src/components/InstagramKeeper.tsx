...
interface InstagramKeeperProps {
  onBack: () => void;
}
...
interface InstagramKeeperSettings {
  themeColor: string;
...
export function InstagramKeeper({ onBack }: InstagramKeeperProps) {
  const [notes, setNotes] = useState<Note[]>([]);
  const [settings, setSettings] = useState<InstagramKeeperSettings>({
  ...
  const loadData = async () => {
    ...
    // Filter for instagram-keeper notes
    const instagramNotes = allEntries.filter((e: any) => e.notebookId === 'instagram-keeper');
    // Sort by date descending
    const sortedEntries = instagramNotes.sort((a: any, b: any) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
    setNotes(sortedEntries);

    const user = await db.getUser();
    if (user && user.instagramKeeperSettings) {
      setSettings(user.instagramKeeperSettings);
    }
  };

  const updateSettings = async (newSettings: Partial<InstagramKeeperSettings>) => {
    const updatedSettings = { ...settings, ...newSettings };
    setSettings(updatedSettings);
    
    const user = await db.getUser() || { id: 'profile' };
    await db.updateUser({ ...user, instagramKeeperSettings: updatedSettings });
  };
    ...
    >
      ...
      <div className="relative shrink-0 z-10">
        ...
        <div className="absolute -bottom-10 left-8 flex items-end space-x-4 z-10">
          ...
          <div className="pb-12">
            <h1 className="text-2xl font-serif font-bold text-stone-800 drop-shadow-sm">Keep Instagram</h1>
